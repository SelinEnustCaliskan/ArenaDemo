        ...
        */
        ...
        protected override int SetArraySizes(int[] chars)
        {
            ...
            for (int i = 0; chars[i] != 0; i++)
            {
                ...
                if (m_currentFontAsset.characterDictionary.TryGetValue(c, out glyph) == false)
                {
                    ...
                    if (m_currentFontAsset.fallbackFontAssets != null && m_currentFontAsset.fallbackFontAssets.Count > 0)
                    {
                        ...
                        for (int y = 0; y < m_currentFontAsset.fallbackFontAssets.Count; y++)
                        {
                            ...
                            if (tempFontAsset.characterDictionary.TryGetValue(c, out glyph))
                            {
                                ...
                                //m_currentMaterial = MaterialManager.GetFallbackMaterial(m_currentMaterial, tempFontAsset.material.mainTexture);
                    ...
                    // Since the missing character is still unavailable, try replacing it by the missing glyph (9633) character.
                    if (glyph == null)
                    {
                        ...
                        else
                        {
                            ...
                            if (glyph == null)
                            {
                                ...
                                {
                                    // Check in the ARIAL SDF Font Asset.
                ...
                if (!char.IsWhiteSpace((char)c))
                {
                    // HANDLE FONT WEIGHTS
                    #region FONT WEIGHTS
                    //if (m_style == FontStyles.Bold || m_fontStyle == FontStyles.Bold)
                    //{
                    //    Debug.Log("Typeface is bold.");
                    //}
                    //else if (m_style == FontStyles.Italic || m_fontStyle == FontStyles.Italic)
                    //{
                    //    Debug.Log("Typeface is italic.");
                    //}
                    #endregion
            ...
            for (int i = 0; i < materialCount; i++)
            {
                ...
                if (i > 0)
                {
                    ...
                    {
                        Material fallbackMaterial = TMP_MaterialManager.GetFallbackMaterial(i == 1 ? m_sharedMaterial : m_subTextObjects[i - 1].sharedMaterial, m_materialReferences[i].material.mainTexture);
                        m_materialReferences[i].material = fallbackMaterial;
                        m_subTextObjects[i].sharedMaterial = fallbackMaterial;
                    }
        ...
        void LateUpdate()
        {
            if (m_rectTransform.hasChanged)
            {
                ...
                if (!m_havePropertiesChanged && currentLossyScale.y != m_previousLossyScale.y && m_text != string.Empty)
                {
        ...
        protected override void GenerateTextMesh()
        {
            ...
            for (int i = 0; m_char_buffer[i] != 0; i++)
            {
                ...
                if (m_textElementType == TMP_TextElementType.Character && ((m_style & FontStyles.Bold) == FontStyles.Bold || (m_fontStyle & FontStyles.Bold) == FontStyles.Bold)) // Checks for any combination of Bold Style.
                {
